...
class States:
	...
	@staticmethod
	def is_safe(x, y):
		if x >= 0 and y >= 0 and (4 - x) >= 0 and (4 - y) >= 0:
			if x == 4:
				if x >= y:
					return True
				else:
	...
	def get_next_action(self):
		...
		action_list = []
		for swimmer in range(1, 4):  # 1-3 people can use the boat.
